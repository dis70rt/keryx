...
async def login_and_save_state(auth_file: Path | str = "data/auth_state.json") -> None:
    ...
    logger.info("Starting Playwright with persistent context for login...")
    async with async_playwright() as p:
        ...
        logger.warn(">>> Please log in manually in the opened browser window. <<<")

        try:
            # Wait until we see evidence of successful login (like the feed URL)
            await page.wait_for_url("**/feed/*", timeout=0)
            await context.storage_state(path=str(auth_path))
            logger.success(f"Login successful! Saved state to -> {auth_path}")
        finally:
            await context.close()
